...
class getBeamSize:
    ...
    def get_beam_size(self):
        ...
        try:
            ...
            if maxbety*maxbetx<10**5:
            
            	x0 = np.array(ptc_output['x'].loc['#e']) #in meters
            	y0 = np.array(ptc_output['y'].loc['#e'])
            	z0 = np.array(ptc_output['t'].loc['#e'])
            	px0 = np.array(ptc_output['px'].loc['#e'])
            	py0 = np.array(ptc_output['py'].loc['#e'])
            	loss = (self.n_particles - len(x0))/self.n_particles
            else:
            	loss=maxbety*maxbetx/10**3
            	_, beam_size_x, beam_size_y, beam_pos_x, beam_pos_y, beam_size_z, _, kl_divergence, \
                sig_nom_x_after, sig_nom_y_after, betx, bety, alfx, alfy, dx, dx2, frac_x, frac_y = self.penalise()
